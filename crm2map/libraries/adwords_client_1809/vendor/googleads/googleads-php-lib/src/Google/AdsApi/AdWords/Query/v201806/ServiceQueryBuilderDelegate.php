...
final class ServiceQueryBuilderDelegate
{
     ...
     */
    public static function copyFrom(ServiceQueryBuilderDelegate $otherInstance, ServiceQueryBuilder $queryBuilder)
    {
        ...
        if (isset($otherInstance->whereBuilders)) {
            ...
            foreach ($otherInstance->whereBuilders as $whereBuilder) {
                $copyingInstance->whereBuilders[] = ServiceQueryWhereBuilder::copyFrom($whereBuilder, $queryBuilder);
            }
        }
    ...
    public function where($field, ServiceQueryBuilder $serviceQueryBuilder)
    {
        ...
        $whereBuilder = ServiceQueryWhereBuilder::createWithField($field, $serviceQueryBuilder);
    ...
    private function orderBy($field, $descOrAsc)
    {
        $validationResult = QueryValidator::validateFieldName($field);
        if ($validationResult->isFailed()) {
            throw new InvalidArgumentException('The field name for building' . ' the ORDER BY clause is invalid. Validation fail reason: ' . $validationResult->getFailReason());
        }
        if (!isset($this->orderByFields)) {
            $this->orderByFields = array();
        }
        if (array_key_exists($field, $this->orderByFields)) {
            throw new InvalidArgumentException('The same field name was added' . ' to the ORDER BY prior to this call. Field names must not' . ' be repeated.');
        }
    ...
    private function appendOrderByClause($awqlString)
    {
        ...
        return sprintf('%s ORDER BY %s', $awqlString, implode(', ', $this->orderByFields));
    }
    /**
    ...
    public function build()
    {
        if (is_null($this->selectFields)) {
            throw new RuntimeException('The `select()` function must be' . ' called prior to this function');
        }
        ...
        return new ServiceQuery($awql, $this->startIndex, $this->pageSize);
    }
}