...
final class ConnectionSettingsBuilder implements AdsBuilder
{
    ...
    public function from(Configuration $configuration)
    {
        ...
        $isReportingGzipEnabled = $configuration->getConfiguration('enableReportingGzip', 'CONNECTION');
    ...
    public function validate()
    {
        if (!empty($this->proxyUrl)) {
            preg_match(ConnectionSettings::PROXY_REGEX_PATTERN, $this->proxyUrl, $matches);
            if (empty($matches)) {
                throw new \InvalidArgumentException('Your specified proxy format is invalid.');
            }
        }
    }
    /**
