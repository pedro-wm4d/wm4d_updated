...
final class ExpressionBuilder
{
    ...
    public function __construct($fieldName)
    {
        ...
        if ($validationResult->isFailed()) {
            throw new InvalidArgumentException('The field name for building a logic expression in the WHERE' . ' clause is invalid. Validation fail reason: ' . $validationResult->getFailReason());
        }
    ...
    {
        if (empty($operator)) {
            throw new InvalidArgumentException('The operator must not be null' . ' or empty.');
        }
        if (is_null($value) || $value === '') {
            throw new InvalidArgumentException('The value string must not be' . ' null or empty');
        }
    ...
    {
        $this->setSimpleOperator(PredicateOperator::STARTS_WITH, $value);
    }
    /**
    ...
    {
        $this->setSimpleOperator(PredicateOperator::STARTS_WITH_IGNORE_CASE, $value);
    }
    /**
    ...
    {
        $this->setSimpleOperator(PredicateOperator::CONTAINS, $value);
    }
    /**
    ...
    {
        $this->setSimpleOperator(PredicateOperator::CONTAINS_IGNORE_CASE, $value);
    }
    /**
    ...
    {
        $this->setSimpleOperator(PredicateOperator::DOES_NOT_CONTAIN, $value);
    }
    /**
    ...
    {
        $this->setSimpleOperator(PredicateOperator::DOES_NOT_CONTAIN_IGNORE_CASE, $value);
    }
    /**
    ...
    private function setListOperator($listOperator, array $values)
    {
        if (empty($listOperator)) {
            throw new InvalidArgumentException('The list operator must not' . ' be null or empty.');
        }
        if (empty($values)) {
            throw new InvalidArgumentException('The list of values must not' . ' be null or empty.');
        }
    ...
    {
        $this->setListOperator(PredicateOperator::IN, $values);
    }
    /**
    ...
    {
        $this->setListOperator(PredicateOperator::NOT_IN, $values);
    }
    /**
    ...
    {
        $this->setListOperator(PredicateOperator::CONTAINS_ANY, $values);
    }
    /**
    ...
    {
        $this->setListOperator(PredicateOperator::CONTAINS_NONE, $values);
    }
    /**
    ...
    {
        $this->setListOperator(PredicateOperator::CONTAINS_ALL, $values);
    }
    /**
    ...
    public function buildExpression()
    {
        if (isset($this->simpleOperator)) {
            return sprintf('%s %s %s', $this->fieldName, $this->simpleOperator, self::formatValue($this->simpleValue));
        }
        if (isset($this->listOperator)) {
            return sprintf('%s %s [%s]', $this->fieldName, $this->listOperator, implode(', ', self::formatValues($this->listValues)));
        }
        throw new BadFunctionCallException('An operator must be set' . ' prior to calling this function.');
    }
}