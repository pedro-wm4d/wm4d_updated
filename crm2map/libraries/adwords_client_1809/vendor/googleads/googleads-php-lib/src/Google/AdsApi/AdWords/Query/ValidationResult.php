...
final class ValidationResult
{
    ...
    private function __construct($passed, $failReason = null)
    {
        if (empty($failReason) !== $passed) {
            throw new InvalidArgumentException('The fail reason must be' . ' specified if and only if the validation failed.');
        }
        if (!empty($failReason) && !is_string($failReason)) {
            throw new InvalidArgumentException('The fail reason must be a' . ' string.');
        }
        $this->passed = $passed;
