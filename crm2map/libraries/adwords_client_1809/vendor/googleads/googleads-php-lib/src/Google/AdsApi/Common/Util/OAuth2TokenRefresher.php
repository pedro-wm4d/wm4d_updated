...
final class OAuth2TokenRefresher
{
    ...
    {
        $this->refreshWindowSeconds = $refreshWindowSeconds === null ? self::DEFAULT_REFRESH_WINDOW_SECONDS : $refreshWindowSeconds;
    }
    /**
     ...
     * @return string
     */
    public function getOrFetchAccessToken(FetchAuthTokenInterface $fetchAuthTokenInterface,/* callable */$httpHandler = null)
    {
        if ($this->shouldFetchAccessToken($fetchAuthTokenInterface)) {
            return ($tmp = $fetchAuthTokenInterface->fetchAuthToken($httpHandler)) ? $tmp['access_token'] : $tmp['access_token'];
        }
        return ($tmp = $fetchAuthTokenInterface->getLastReceivedToken()) ? $tmp['access_token'] : $tmp['access_token'];
    }
    private function shouldFetchAccessToken(FetchAuthTokenInterface $fetchAuthTokenInterface)
    {
        $token = $fetchAuthTokenInterface->getLastReceivedToken();
        return $token === null || $token['expires_at'] !== null && $token['expires_at'] - time() <= $this->refreshWindowSeconds;
    }
    /**
