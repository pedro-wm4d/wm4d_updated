...
final class AdWordsNormalizer extends GetSetMethodNormalizer
{
    ...
    public function normalize($object, $format = null, array $context = array())
    {
        ...
        foreach ($reflClass->getProperties(\ReflectionProperty::IS_PROTECTED | \ReflectionProperty::IS_PRIVATE) as $reflProperty) {
            ...
            if ($attributeValue !== null) {
                if (array_key_exists('normalize', $this->callbacks)) {
                    $attributeValue = call_user_func($this->callbacks['normalize'], $attributeValue, $this->getReturnType($this->docBlockFactory->create($reflMethod->getDocComment())));
                }
    ...
    private function getReturnType(DocBlock $docBlock)
    {
        ...
        return $docBlock->hasTag('return') ? strval(($tmp = $docBlock->getTags()) ? $tmp[0]->getType() : $tmp[0]->getType()) : null;
    }
    /**
    ...
    public function denormalize($data, $class, $format = null, array $context = array())
    {
        ...
        if (array_key_exists('@xsi:type', $data)) {
            ...
            $class = $this->getAdWordsNamespacePrefix($class) . '\\' . $data['@xsi:type'];
        }
        ...
        foreach ($data as $attribute => $value) {
            ...
            if (!$reflClass->hasMethod('get' . ucfirst($attribute))) {
                throw new UnexpectedValueException(sprintf('Getter for the attribute "%s" not found. Denormalization cannot' . ' be completed.', $attribute));
            }
            // Get the type of this attribute using PHP document block of its getter.
            $getter = $reflClass->getMethod('get' . ucfirst($attribute));
            $typeHint = $this->getReturnType($this->docBlockFactory->create($getter->getDocComment()));
            if (array_key_exists('denormalize', $this->callbacks)) {
                $value = call_user_func($this->callbacks['denormalize'], $value, $typeHint);
            }
            ...
            // not have done so.
            if (substr($typeHint, -2) === '[]' && (is_scalar($value) || self::isOneOrMany($value))) {
                $value = array($value);
            }
            if (self::needsRecursiveDenormalization($value)) {
                $value = $this->serializer->denormalize($value, $typeHint, $format, $context);
            }
    ...
    {
        return count($array) === 0 ? false : array_keys($array) !== range(0, count($array) - 1);
    }
    ...
    public function setCallbacks(array $callbacks)
    {
        if (count($callbacks) > 2) {
            throw new InvalidArgumentException(sprintf('More than two callbacks found. Only two callbacks, normalize and' . ' denormalize are allowed.'));
        }
        foreach ($callbacks as $attribute => $callback) {
            ...
            if ($attribute !== 'normalize' && $attribute !== 'denormalize') {
                throw new InvalidArgumentException(sprintf('The given callback "%s" is not supported.', $attribute));
            }
            $reflectionFunction = new \ReflectionFunction($callback);
            if ($reflectionFunction->getNumberOfRequiredParameters() !== 2) {
                throw new InvalidArgumentException(sprintf('Callback "%s" does not meet the expected method signature.', $attribute));
            }
        }
