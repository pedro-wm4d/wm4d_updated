...
final class ReportQueryBuilderDelegate
{
     ...
     */
    public static function copyFrom(ReportQueryBuilderDelegate $otherInstance, ReportQueryBuilder $queryBuilder)
    {
        ...
        $copyingInstance->duringDateRangeType = $otherInstance->duringDateRangeType;
        ...
        if (isset($otherInstance->whereBuilders)) {
            $copyingInstance->whereBuilders = array();
            foreach ($otherInstance->whereBuilders as $whereBuilder) {
                $copyingInstance->whereBuilders[] = ReportQueryWhereBuilder::copyFrom($whereBuilder, $queryBuilder);
            }
        }
    ...
    {
        if (empty($fields)) {
            throw new InvalidArgumentException('The field array must not be' . ' empty.');
        }
        foreach ($fields as $field) {
            $validationResult = QueryValidator::validateFieldName($field);
            if ($validationResult->isFailed()) {
                throw new InvalidArgumentException('The field array for' . ' building the SELECT clause contains invalid field name.' . ' Validation fail reason: ' . $validationResult->getFailReason());
            }
        }
    ...
    {
        if (empty($reportType)) {
            throw new InvalidArgumentException('The report type must not be' . ' null or empty.');
        }
    ...
    public function where($field, ReportQueryBuilder $reportQueryBuilder)
    {
        if (is_null($this->whereBuilders)) {
            $this->whereBuilders = array();
        }
        $whereBuilder = ReportQueryWhereBuilder::createWithField($field, $reportQueryBuilder);
    ...
    public function duringDateRange($dateRangeType)
    {
        if (empty($dateRangeType)) {
            throw new InvalidArgumentException('The date range type must not' . ' be null or empty.');
        }
    ...
    {
        $validationResult = QueryValidator::validateCustomDateRange($startDate, $endDate);
        if ($validationResult->isFailed()) {
            throw new InvalidArgumentException('The start or end date is' . ' invalid. Validation fail reasons: ' . $validationResult->getFailReason());
        }
    ...
    {
        return WhereClauseAppender::appendWhereClause($this->whereBuilders, $awql);
    }
    /**
    ...
    private function appendDuringClause($awql)
    {
        ...
        if (isset($this->duringStartDate) && isset($this->duringEndDate)) {
            return sprintf('%s DURING %s,%s', $awql, $this->duringStartDate, $this->duringEndDate);
        }
        return $awql;
    }
    /**
    ...
    public function build()
    {
        if (is_null($this->selectFields) || is_null($this->fromReportType)) {
            throw new RuntimeException('The `select()` and `from()` functions' . ' must be called prior to this function');
        }
