...
final class MapEntries
{
    ...
    public static function toAssociativeArray(array $mapEntriesList)
    {
        ...
        foreach ($mapEntriesList as $mapEntry) {
            try {
                ...
                throw new UnexpectedValueException('Each map entry type must contain a \'key\' property and a \'value\' property.');
            }
        }
        return $result;
    }
     ...
     */
    public static function fromAssociativeArray(array $mapEntriesAssociativeArray, $mapEntryClassName)
    {
        $result = array();
        foreach ($mapEntriesAssociativeArray as $key => $value) {
            try {
                ...
                throw new UnexpectedValueException(sprintf('The map entry class \'%s\' could not be found.', $mapEntryClassName));
            }
            try {
                $keyProperty = $reflectionObject->getProperty('key');
                $keyProperty->setAccessible(true);
                ...
                throw new UnexpectedValueException('The map entry class must contains a \'key\' property and a \'value\' ' . 'property.');
            }
            $result[] = $entry;
        }
        return $result;
