...
final class AdWordsHeaderHandler implements AdsHeaderHandler
{
    ...
    public function generateHttpHeaders(AdsSession $session)
    {
        ...
        $httpHeaders = array('Authorization' => sprintf('Bearer %s', urlencode($this->oAuth2TokenRefresher->getOrFetchAccessToken($session->getOAuth2Credential(), $httpHandler))));
        return $httpHeaders;
    }
    /**
     * @see AdsHeaderHandler::generateSoapHeaders()
     */
    public function generateSoapHeaders(AdsSession $session, AdsServiceDescriptor $serviceDescriptor)
    {
        ...
        $soapRequestHeader->setUserAgent($this->formatUserAgentForSoapHeader($session->getUserAgent(), $session->isIncludeUtilitiesInUserAgent()));
        ...
        return new SoapHeader($serviceDescriptor->getSoapNamespace(), self::SOAP_HEADER_NAME, $soapRequestHeader);
    }
    /**
     * Creates a new instance of the SOAP header object used by the AdWords API.
     */
    private function createSoapHeaderObject(AdsServiceDescriptor $serviceDescriptor)
    {
        ...
        $namespaceParts = explode('\\', trim($reflectionClass->getNamespaceName(), '\\'));
        // Remove the group.
        array_pop($namespaceParts);
        $soapHeaderClassName = sprintf('%s\\%s', implode('\\', $namespaceParts), self::DEFAULT_SOAP_HEADER_CLASS_NAME);
        return $this->reflection->createInstance($soapHeaderClassName);
    }
    private function formatUserAgentForSoapHeader($userAgent, $includeUtilityUsage)
    {
        return $this->adsHeaderFormatter->formatApplicationNameForSoapHeader($userAgent, self::PRODUCT_NAME_FOR_SOAP_HEADER, $includeUtilityUsage);
    }
}