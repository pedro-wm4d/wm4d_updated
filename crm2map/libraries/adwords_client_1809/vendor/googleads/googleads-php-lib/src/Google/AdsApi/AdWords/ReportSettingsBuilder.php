...
final class ReportSettingsBuilder implements AdsBuilder
{
    ...
    public function __construct(ReportSettings $reportSettings = null)
    {
        ...
        if (!is_null($reportSettings)) {
            ...
            $this->isIncludeZeroImpressions = $reportSettings->isIncludeZeroImpressions();
        }
    }
    /**
     * @see AdsBuilder::from()
    ...
    {
        $isSkipReportHeader = $configuration->getConfiguration('isSkipReportHeader', 'ADWORDS_REPORTING');
        if ($isSkipReportHeader !== null) {
            $this->isSkipReportHeader = filter_var($isSkipReportHeader, FILTER_VALIDATE_BOOLEAN);
        }
        $isSkipColumnHeader = $configuration->getConfiguration('isSkipColumnHeader', 'ADWORDS_REPORTING');
        if ($isSkipColumnHeader !== null) {
            $this->isSkipColumnHeader = filter_var($isSkipColumnHeader, FILTER_VALIDATE_BOOLEAN);
        }
        $isSkipReportSummary = $configuration->getConfiguration('isSkipReportSummary', 'ADWORDS_REPORTING');
        if ($isSkipReportSummary !== null) {
            $this->isSkipReportSummary = filter_var($isSkipReportSummary, FILTER_VALIDATE_BOOLEAN);
        }
        $isUseRawEnumValues = $configuration->getConfiguration('isUseRawEnumValues', 'ADWORDS_REPORTING');
        if ($isUseRawEnumValues !== null) {
            $this->isUseRawEnumValues = filter_var($isUseRawEnumValues, FILTER_VALIDATE_BOOLEAN);
        }
        return $this;
    }
    /**
