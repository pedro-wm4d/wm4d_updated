...
final class SoapLogMessageFormatter
{
     ...
     */
    public function __construct(array $requestHttpHeadersToScrub = null, array $requestSoapHeadersToScrub = null, array $requestSoapBodyTagsToScrub = null, array $additionalRequestSummaryHeaders = null, array $additionalResponseSummaryHeaders = null, $faultMsgMaxLength = null)
    {
        $this->requestHttpHeadersToScrub = $requestHttpHeadersToScrub === null ? array() : $requestHttpHeadersToScrub;
        $this->requestSoapHeadersToScrub = $requestSoapHeadersToScrub === null ? array() : $requestSoapHeadersToScrub;
        $this->requestSoapBodyTagsToScrub = $requestSoapBodyTagsToScrub === null ? array() : $requestSoapBodyTagsToScrub;
        $this->additionalRequestSummaryHeaders = $additionalRequestSummaryHeaders === null ? array() : $additionalRequestSummaryHeaders;
        $this->additionalResponseSummaryHeaders = $additionalResponseSummaryHeaders === null ? array() : $additionalResponseSummaryHeaders;
        $this->faultMsgMaxLength = $faultMsgMaxLength === null ? self::$FAULT_MSG_MAX_LEN_DEFAULT : $faultMsgMaxLength;
    }
    /**
     ...
     */
    public function formatSummary($serviceName, $methodName, $requestHeaders, $request, $response, $soapFault = null)
    {
        $additionalHeadersFormatted = array();
        foreach ($this->additionalRequestSummaryHeaders as $additionalReqHeader) {
            $additionalHeadersFormatted[] = sprintf('%s=%s ', $additionalReqHeader, SoapHeaders::getSoapHeaderValue($request, $additionalReqHeader));
        }
        foreach ($this->additionalResponseSummaryHeaders as $additionalRespHeader) {
            $additionalHeadersFormatted[] = sprintf('%s=%s ', $additionalRespHeader, SoapHeaders::getSoapHeaderValue($response, $additionalRespHeader));
        }
        return sprintf('%sservice=%s method=%s responseTime=%s requestId=%s server=%s ' . 'isFault=%b faultMessage=%s', implode('', $additionalHeadersFormatted), $serviceName, $methodName, SoapHeaders::getSoapHeaderValue($response, 'responseTime'), SoapHeaders::getSoapHeaderValue($response, 'requestId'), $this->getServer($requestHeaders), $soapFault !== null, $soapFault !== null ? $this->formatSoapFaultForSummary($soapFault->getMessage()) : '');
    }
    /**
     ...
     */
    public function formatDetailed($requestHeaders, $request, $responseHeaders, $response)
    {
        $requestHeaders = LogMessageScrubbers::scrubRequestHttpHeaders(trim($requestHeaders), $this->requestHttpHeadersToScrub);
        $request = LogMessageScrubbers::scrubRequestSoapHeaders($request, $this->requestSoapHeadersToScrub);
        $request = LogMessageScrubbers::scrubRequestSoapBodyTags($request, $this->requestSoapBodyTagsToScrub);
...
', ''), '', $soapFault);
    ...
    {
        $hostMatches = array();
        $isMatch = preg_match('/^.*Host:\\s(.*)Connection:.*$/s', $requestHeaders, $hostMatches);
        if ($isMatch) {
