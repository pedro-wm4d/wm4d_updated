...
final class ReportQuery
{
     ...
     */
    public function __construct($awqlString)
    {
        $validationResult = QueryValidator::validateReportQuery($awqlString);
        if ($validationResult->isFailed()) {
            throw new InvalidArgumentException('The report query string is invalid. Validation fail reason: ' . $validationResult->getFailReason());
        }
