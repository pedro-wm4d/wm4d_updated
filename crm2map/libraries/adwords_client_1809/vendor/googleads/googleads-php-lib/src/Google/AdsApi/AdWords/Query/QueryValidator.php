...
final class QueryValidator
{
    ...
    public static function validateFieldName($fieldName)
    {
        if (!is_string($fieldName)) {
            return ValidationResult::fail('The field name must be a' . ' string.');
        }
/*
        if (empty(trim($fieldName))) {		
            return ValidationResult::fail('The field name must not be' . ' null, empty, white spaces or zero.');
        } 
*/
     ...
     *     is passed or failed with detail explanation
     */
    public static function validateCustomDateRange($startDateString, $endDateString)
    {
        ...
        if (false === $startDate || $warningsFound) {
            return ValidationResult::fail('The start date must be a valid' . ' date and follow YYYYMMDD format.');
        }
        $endDate = DateTime::createFromFormat('Ymd', $endDateString);
        $warningsFound = !empty(DateTime::getLastErrors()['warnings']);
        if (false === $endDate || $warningsFound) {
            return ValidationResult::fail('The end date must be a valid date' . ' and follow YYYYMMDD format.');
        }
        if ($endDate < $startDate) {
            return ValidationResult::fail('The end date must not be prior to' . ' the start date.');
        }
        return ValidationResult::pass();
    }
    /**
     ...
     *     returns false
     */
    private static function containKeywords($awqlString, $clauseKeywords, $containAll)
    {
        ...
        $awqlStringWithoutLiteralStrings = preg_replace($patterns, self::REDACTED_LITERAL_STRING, $awqlString);
        $count = 0;
    ...
    {
        return self::containKeywords($awqlString, $clauseKeywords, true);
    }
    /**
    ...
    {
        return self::containKeywords($awqlString, $clauseKeywords, false);
    }
    /**
    ...
    {
        if (empty($awqlString)) {
            return ValidationResult::fail('The AWQL string must not be' . ' null or empty');
        }
        if (!self::containAllKeywords($awqlString, $requiredKeywords)) {
            return ValidationResult::fail(sprintf('The AWQL string must contain %s clause(s).', implode(', ', $requiredKeywords)));
        }
        if (self::containAnyKeywords($awqlString, $forbiddenKeywords)) {
            return ValidationResult::fail(sprintf('The AWQL string must not contain %s clauses.', implode(', ', $forbiddenKeywords)));
        }
        return ValidationResult::pass();
    }
    /**
     ...
     *     is passed or failed with detail explanation
     */
    public static function validateServiceQuery($awqlString, $allowPagination = true)
    {
        $forbiddenKeywords = array('FROM', 'DURING');
        if (!$allowPagination) {
            $forbiddenKeywords[] = 'LIMIT';
        }
        return self::validateQuery($awqlString, array('SELECT'), $forbiddenKeywords);
    }
    /**
    ...
    {
        return self::validateQuery($awqlString, array('SELECT', 'FROM'), array('ORDER BY', 'LIMIT'));
    }
}