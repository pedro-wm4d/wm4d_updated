...
final class BatchJobsDelegate
{
    ...
    private static $MUTATE_RESULT_CLASS_NAME = 'Google\\AdsApi\\AdWords\\v201806\\cm\\MutateResult';
    private $session;
    private $httpClient;
     ...
     *     calls
     */
    public function __construct(AdWordsSession $session, Client $httpClient = null, $mutateResultClassName = null, GuzzleHttpClientFactory $httpClientFactory = null)
    {
        $this->session = $session;
        if ($httpClientFactory === null) {
            $logMessageFormatterProvider = new AdWordsGuzzleLogMessageFormatterProvider($session, true);
            $this->httpClientFactory = new AdsGuzzleHttpClientFactory($session->getBatchJobsUtilLogger(), $logMessageFormatterProvider->getGuzzleLogMessageFormatter(), $httpClient);
        } else {
            $this->httpClientFactory = $httpClientFactory;
        }
        $this->httpClient = $this->httpClientFactory->generateHttpClient();
        $this->mutateResultClassName = $mutateResultClassName === null ? self::$MUTATE_RESULT_CLASS_NAME : $mutateResultClassName;
        $adWordsNormalizer = new AdWordsNormalizer(null, new DotRemoverNameConverter());
        // Set callbacks for the normalizer to specially handle serialization and
        // deserialization of boolean variables.
        $adWordsNormalizer->setCallbacks(array('normalize' => function ($value, $type) {
        ...
        $this->batchJobSerializer = new Serializer(array($adWordsNormalizer, new ArrayDenormalizer()), array(new XmlEncoder('ns1:mutate')));
    }
    /**
    ...
    {
        return $this->closeIncrementalUpload($this->uploadIncrementalBatchJobOperations($operations, new BatchJobUploadStatus($uploadUrl, $this->session, 0, $this->httpClient, $this->httpClientFactory)));
    }
    /**
     * @see BatchJobs::uploadIncrementalBatchJobOperations
     */
    public function uploadIncrementalBatchJobOperations(array $operations, BatchJobUploadStatus $batchJobUploadStatus)
    {
        $serializedContent = $this->batchJobSerializer->serialize(array('operations' => $operations, '@xmlns:ns1' => 'https://adwords.google.com/api/adwords/cm/v201806', '@xmlns:xsi' => 'http://www.w3.org/2001/XMLSchema-instance'), 'xml');
        // Remove the XML opening tag in case this is not the first request.
        if ($batchJobUploadStatus->getTotalContentBytes() > 0) {
            $serializedContent = preg_replace('/^.*?<operations/s', '<operations', $serializedContent);
        }
        // Always remove the XML closing tag. It will be added when
        // closeIncrementalUpload is called.
        $serializedContent = preg_replace('/<\\/ns1:mutate>\\s*$/s', '', $serializedContent);
        ...
        $upperBound = $batchJobUploadStatus->getTotalContentBytes() + $contentLength - 1;
        $contentRange = sprintf('bytes %d-%d/*', $lowerBound, $upperBound);
        try {
            $requestOptions = array();
            $requestOptions[RequestOptions::BODY] = $serializedContent;
            $requestOptions[RequestOptions::HEADERS] = array('Content-Length' => $contentLength, 'Content-Range' => $contentRange, 'Content-Type' => 'application/xml');
            $proxy = $this->session->getConnectionSettings()->getProxyUrl();
            if (!empty($proxy)) {
                $requestOptions[RequestOptions::PROXY] = array('https' => $proxy);
            }
            $this->httpClient->request('PUT', $batchJobUploadStatus->getResumableUploadUrl(), $requestOptions);
        } catch (ServerException $e) {
            throw new ApiException(sprintf('Failed response status from batch upload URL, error message: %s,' . ' HTTP code %d.', $e->getResponse()->getReasonPhrase(), $e->getResponse()->getStatusCode()));
        }
        return new BatchJobUploadStatus($batchJobUploadStatus->getResumableUploadUrl(), $this->session, $batchJobUploadStatus->getTotalContentBytes() + $contentLength, $this->httpClient, $this->httpClientFactory);
    }
    /**
     * @see BatchJobs::closeIncrementalUpload
     */
    public function closeIncrementalUpload(BatchJobUploadStatus $batchJobUploadStatus)
    {
        $content = '</ns1:mutate>';
    ...
    public function downloadBatchJobResults($downloadUrl)
    {
        try {
            $requestOptions = array();
            $requestOptions[RequestOptions::HEADERS] = array('Accept-Encoding' => 'gzip');
            $proxy = $this->session->getConnectionSettings()->getProxyUrl();
            if (!empty($proxy)) {
                $requestOptions[RequestOptions::PROXY] = array('https' => $proxy);
            }
            $response = $this->httpClient->request('GET', $downloadUrl, $requestOptions);
        } catch (ServerException $e) {
            throw new ApiException(sprintf('Failed response status from batch download URL, error message: %s,' . ' HTTP code %d.', $e->getResponse()->getReasonPhrase(), $e->getResponse()->getStatusCode()));
        }
        $mutateResponse = $this->batchJobSerializer->decode($response->getBody()->getContents(), 'xml');
        if ($mutateResponse === '') {
            return array();
        }
        $rval = AdWordsNormalizer::isOneOrMany($mutateResponse['rval']) ? array($mutateResponse['rval']) : $mutateResponse['rval'];
        return $this->batchJobSerializer->denormalize($rval, $this->mutateResultClassName . '[]');
    }
    /**
    ...
    private function padContent($content)
    {
        ...
        if ($remainder > 0) {
            $targetLength = $numBytes + (self::$REQUIRED_CONTENT_BYTES_INCREMENT - $remainder);
