...
final class GuzzleLogMessageHandler
{
     ...
     */
    public static function log(LoggerInterface $logger, GuzzleLogMessageFormatter $messageFormatter)
    {
        return function (/*callable */$handler) use($logger, $messageFormatter) {
            return function ($request, array $options) use($handler, $logger, $messageFormatter) {
                return $handler($request, $options)->then(function ($response) use($request, $logger, $messageFormatter) {
                    // Logs messages in case of successful HTTP calls.
                    $logger->info($messageFormatter->formatSummary($request, $response));
                    $logResponse = $response !== null && $response->getBody()->isSeekable() ? $response : null;
                    if (!$logger instanceof Logger || $logger->isHandling(Logger::DEBUG)) {
                        // formatDetailed() can produce long log messages so check
                        // if it handles DEBUG level when possible.
                        $logger->debug($messageFormatter->formatDetailed($request, $logResponse));
                    }
                    if ($logResponse !== null) {
                    ...
                    $response = is_subclass_of($reason, 'GuzzleHttp\\Exception\\RequestException') ? $reason->getResponse() : null;
                    $logger->warning($messageFormatter->formatSummary($request, $response));
                    $logResponse = $response !== null && $response->getBody()->isSeekable() ? $response : null;
                    if (!$logger instanceof Logger || $logger->isHandling(Logger::NOTICE)) {
                        // formatDetailed() can produce long log messages so check
                        // if it handles NOTICE level when possible.
                        $logger->notice($messageFormatter->formatDetailed($request, $logResponse, $reason));
                    }
                    if ($logResponse !== null) {
                        // Rewind the response body if it was once read by
                        // formatDetailed() so the caller of this method can use the
