...
{
    private static $REPORT_DOWNLOAD_URL_PATH = '/api/adwords/reportdownload/v201809';
     ...
     *     calls
     */
    public function __construct(AdWordsSession $session, RequestOptionsFactory $requestOptionsFactory = null, Client $httpClient = null, GuzzleHttpClientFactory $httpClientFactory = null)
    {
        $this->session = $session;
        if ($httpClientFactory === null) {
            $logMessageFormatterProvider = new AdWordsGuzzleLogMessageFormatterProvider($session, false, self::$REDACTED_DATA_MESSAGE);
            $httpClientFactory = new AdsGuzzleHttpClientFactory($this->session->getReportDownloaderLogger(), $logMessageFormatterProvider->getGuzzleLogMessageFormatter(), $httpClient);
        }
        $this->httpClient = $httpClientFactory->generateHttpClient();
        $this->requestOptionsFactory = $requestOptionsFactory === null ? new RequestOptionsFactory($session) : $requestOptionsFactory;
        $this->apiErrorSerializer = new Serializer(array(new GetSetMethodNormalizer(null, new ApiErrorFieldNameConverter()), new ArrayDenormalizer()), array(new XmlEncoder()));
    }
    /**
     ...
     */
    public function downloadReport(ReportDefinition $reportDefinition, ReportSettings $reportSettingsOverride = null)
    {
        return $this->makeReportRequest($this->requestOptionsFactory->createRequestOptionsWithReportDefinition($reportDefinition, $reportSettingsOverride));
    }
    /**
     ...
     * @throws ApiException if there are errors during downloading reports
     */
    public function downloadReportWithAwql($reportQuery, $reportFormat, ReportSettings $reportSettingsOverride = null)
    {
        ...
        if (!is_string($reportQuery)) {
            throw new InvalidArgumentException('The report query must be a' . ' string.');
        }
        return $this->makeReportRequest($this->requestOptionsFactory->createRequestOptionsWithAwqlQuery($reportQuery, $reportFormat, $reportSettingsOverride));
    }
    /**
     ...
     * @throws ApiException if there are errors during downloading reports
     */
    public function downloadReportWithReportQuery(ReportQuery $reportQuery, $reportFormat, ReportSettings $reportSettingsOverride = null)
    {
        $awqlString = sprintf('%s', $reportQuery);
        return $this->downloadReportWithAwql($awqlString, $reportFormat, $reportSettingsOverride);
    }
    /**
    ...
    private function makeReportRequest(array $requestOptions)
    {
        ...
        if ($this->session->getConnectionSettings()->isReportingGzipEnabled() === true) {
            $requestOptions[RequestOptions::DECODE_CONTENT] = 'gzip';
        }
        try {
            $response = $this->httpClient->request('POST', rtrim($this->session->getEndpoint(), '/') . self::$REPORT_DOWNLOAD_URL_PATH, $requestOptions);
        } catch (ClientException $e) {
            $decodedResponse = $this->apiErrorSerializer->decode($e->getResponse()->getBody()->getContents(), 'xml');
            ...
            $apiErrorList = AdWordsNormalizer::isOneOrMany($decodedResponse['ApiError']) ? array($decodedResponse['ApiError']) : $decodedResponse['ApiError'];
            $reportDownloadErrors = $this->apiErrorSerializer->denormalize($apiErrorList, 'Google\\AdsApi\\AdWords\\Reporting\\v201809\\ReportDownloadError[]');
            $reportDownloadException = new ApiException(sprintf('Details: [%s]', implode(', ', $reportDownloadErrors)));
            $reportDownloadException->setErrors($reportDownloadErrors);
            throw $reportDownloadException;
        } catch (ServerException $e) {
            throw new ApiException('Temporary problem with the server. Please retry the request' . ' after a few moments');
        }
