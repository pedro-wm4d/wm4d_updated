...
class RequestOptionsFactory
{
     ...
     */
    public function __construct(AdWordsSession $session, array $options = null, Serializer $reportDefinitionSerializer = null)
    {
        $this->session = $session;
        $this->reportDefinitionSerializer = $reportDefinitionSerializer === null ? new Serializer(array(new AdWordsNormalizer()), array(new XmlEncoder())) : $reportDefinitionSerializer;
        $this->oAuth2TokenRefresher = new OAuth2TokenRefresher();
        $this->options = $options === null ? array('stream_context' => array('http' => array('timeout' => self::DEFAULT_TIMEOUT_IN_SECONDS))) : $options;
        $this->httpHandler = new AdsGuzzleProxyHttpHandler($session);
    }
    private function createHeaders(ReportSettings $reportSettingsOverride = null)
    {
        $headers = array('Authorization' => 'Bearer ' . urlencode($this->oAuth2TokenRefresher->getOrFetchAccessToken($this->session->getOAuth2Credential(), $this->httpHandler)), 'developerToken' => $this->session->getDeveloperToken(), 'clientCustomerId' => $this->session->getClientCustomerId(), 'User-Agent' => $this->session->getAdsHeaderFormatter()->formatApplicationNameForGuzzleHeader($this->session->getUserAgent(), AdWordsHeaderHandler::PRODUCT_NAME_FOR_SOAP_HEADER, $this->session->isIncludeUtilitiesInUserAgent(), $this->session->getConnectionSettings()->isReportingGzipEnabled()));
        ...
        if (!is_null($includeZeroImpressions)) {
            $headers['includeZeroImpressions'] = $includeZeroImpressions ? 'true' : 'false';
        }
        return $headers;
    }
    /**
     ...
     */
    public function createRequestOptionsWithReportDefinition(ReportDefinition $reportDefinition, ReportSettings $reportSettingsOverride = null)
    {
        $params = array();
        $context = array('xml_root_node_name' => 'reportDefinition');
        $params['__rdxml'] = $this->reportDefinitionSerializer->serialize($reportDefinition, 'xml', $context);
        return array_merge($this->options, array(RequestOptions::HEADERS => $this->createHeaders($reportSettingsOverride), RequestOptions::FORM_PARAMS => $params));
    }
    /**
     ...
     * @return array the request options
     */
    public function createRequestOptionsWithAwqlQuery($reportDefinition, $reportFormat, ReportSettings $reportSettingsOverride = null)
    {
        $params = array('__rdquery' => $reportDefinition, '__fmt' => $reportFormat);
        return array_merge($this->options, array(RequestOptions::HEADERS => $this->createHeaders($reportSettingsOverride), RequestOptions::FORM_PARAMS => $params));
    }
}