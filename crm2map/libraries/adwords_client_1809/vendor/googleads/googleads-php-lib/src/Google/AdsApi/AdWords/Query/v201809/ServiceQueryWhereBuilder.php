...
final class ServiceQueryWhereBuilder implements WhereBuilderInterface
{
     ...
     */
    private function __construct(ServiceQueryBuilder $queryBuilder, ExpressionBuilder $expressionBuilder)
    {
     ...
     */
    public static function createWithField($field, ServiceQueryBuilder $queryBuilder)
    {
        $validationResult = QueryValidator::validateFieldName($field);
        if ($validationResult->isFailed()) {
            throw new InvalidArgumentException('The field name for building the WHERE clause is invalid.' . ' Validation fail reason: ' . $validationResult->getFailReason());
        }
     ...
     */
    public static function copyFrom(ServiceQueryWhereBuilder $otherInstance, ServiceQueryBuilder $queryBuilder)
    {
        return new self($queryBuilder, ExpressionBuilder::copyFrom($otherInstance->expressionBuilder));
    }
    /**
