...
final class BatchJobUploadStatus
{
     ...
     *     calls
     */
    public function __construct($url, AdWordsSession $session, $totalContentBytes = null, Client $httpClient = null, GuzzleHttpClientFactory $httpClientFactory = null)
    {
        ...
        if ($httpClientFactory === null) {
            ...
            $httpClientFactory = new AdsGuzzleHttpClientFactory($session->getBatchJobsUtilLogger(), $logMessageFormatterProvider->getGuzzleLogMessageFormatter(), $httpClient);
        }
        $this->httpClient = $httpClientFactory->generateHttpClient();
        $this->totalContentBytes = $totalContentBytes === null ? 0 : $totalContentBytes;
        // If this is the first upload, then issue a request to get the resumable
        // session URI from Google Cloud Storage.
        $this->resumableUploadUrl = $this->totalContentBytes === 0 ? $this->initiateResumableUpload($url) : $url;
    }
    /**
    ...
    private function initiateResumableUpload($uploadUrl)
    {
        ...
        $requestOptions[RequestOptions::HEADERS] = array('Content-Type' => 'application/xml', 'Content-Length' => 0, 'x-goog-resumable' => 'start');
        if (!empty($this->connectionSettings->getProxyUrl())) {
            $requestOptions[RequestOptions::PROXY] = array('https' => $this->connectionSettings->getProxyUrl());
        }
        ...
        return ($tmp = $response->getHeader('Location')) ? $tmp[0] : $tmp[0];
    }
    /**
