...
final class AdsHeaderFormatter
{
     ...
     *     metadata provider
     */
    public function __construct(AdsUtilityRegistry $adsUtilityRegistry = null, LibraryMetadataProvider $libraryMetadataProvider = null)
    {
        $this->adsUtilityRegistry = $adsUtilityRegistry === null ? AdsUtilityRegistry::getInstance() : $adsUtilityRegistry;
        $this->libraryMetadataProvider = $libraryMetadataProvider === null ? new LibraryMetadataProvider() : $libraryMetadataProvider;
    }
    /**
     ...
     */
    public function formatApplicationNameForSoapHeader($applicationName, $productNameForSoapHeader, $includeUtilityUsage)
    {
        $adsUtilities = $this->adsUtilityRegistry->popAllUtilities();
        return sprintf('%s (%sApi-PHP, %s/%s, PHP/%s%s)', $applicationName, $productNameForSoapHeader, $this->libraryMetadataProvider->getLibName(), $this->libraryMetadataProvider->getLibVersion(), PHP_VERSION, $this->formatUtilUsages($adsUtilities, $includeUtilityUsage));
    }
    /**
     * Formats an application name in a format standard to the ads libraries to
     ...
     * @return string the formatted application name
     */
    public function formatApplicationNameForGuzzleHeader($applicationName, $productName, $includeUtilityUsage, $isReportingGzipEnabled = null)
    {
        $adsUtilities = $this->adsUtilityRegistry->popAllUtilities();
        return sprintf('%s (%sApi-PHP, %s/%s, PHP/%s, %s%s%s)', $applicationName, $productName, $this->libraryMetadataProvider->getLibName(), $this->libraryMetadataProvider->getLibVersion(), PHP_VERSION, $this->formatGuzzleInfo(), $this->formatUtilUsages($adsUtilities, $includeUtilityUsage), $isReportingGzipEnabled === true ? ', gzip' : '');
    }
    ...
    {
        $guzzleInfoTokens = array('GuzzleHttp/' . ClientInterface::VERSION);
        if (extension_loaded('curl') && function_exists('curl_version')) {
            $guzzleInfoTokens[] = 'curl/' . (($tmp = \curl_version()) ? $tmp['version'] : $tmp['version']);
        }
