...
final class ServiceQuery
{
     ...
     *     each page
     */
    public function __construct($awqlString, $startIndex = null, $pageSize = null)
    {
    ...
    {
        $validationResult = QueryValidator::validateServiceQuery($awqlString, false);
        if ($validationResult->isFailed()) {
            throw new InvalidArgumentException('The service query string is invalid. Validation fail reason: ' . $validationResult->getFailReason());
        }
        ...
        if (is_null($startIndex) !== is_null($pageSize)) {
            throw new InvalidArgumentException('Start index and page size' . ' must be both null, or both not null.');
        }
        if (!is_null($startIndex)) {
            if (!is_int($startIndex)) {
                throw new InvalidArgumentException('The start index must be' . ' an integer.');
            }
            if ($startIndex < 0) {
                throw new OutOfBoundsException('The start index must be 0 or' . ' a positive number.');
            }
        }
        if (!is_null($pageSize)) {
            if (!is_int($pageSize)) {
                throw new InvalidArgumentException('The page size must be' . ' an integer.');
            }
            if ($pageSize < 1) {
                throw new OutOfBoundsException('The page size must be a' . ' positive number.');
            }
        }
    }
    /**
    ...
    public function __toString()
    {
        if (!is_null($this->startIndex) && !is_null($this->pageSize)) {
            return sprintf('%s LIMIT %d,%d', $this->awqlString, $this->startIndex, $this->pageSize);
        }
    ...
    public function nextPage(Page $previousPage = null)
    {
        ...
        if (!$previousPage instanceof AdGroupBidLandscapePage && !$previousPage instanceof CriterionBidLandscapePage) {
            throw new InvalidArgumentException('The page object must be an' . ' instance of either `AdGroupBidLandscapePage` or' . ' `CriterionBidLandscapePage` type');
        }
        $this->startIndex = $this->startIndex + self::countLandscapePoints($previousPage);
        return $this;
    }
    /**
