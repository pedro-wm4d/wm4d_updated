 ...
 */
namespace Google\AdsApi\AdWords\Shopping\v201809;
...
final class ProductPartitions
{
     ...
     */
    public static function createSubdivision(ProductPartition $parent = null, ProductDimension $value = null)
    {
     ...
     * @return ProductPartition the newly created product partition
     */
    public static function createUnit(ProductPartition $parent = null, ProductDimension $value = null)
    {
     ...
     */
    public static function asBiddableAdGroupCriterion($adGroupId, ProductPartition $productPartition, $bidAmount = null)
    {
        ...
        if ($bidAmount !== null && $bidAmount > 0) {
            ...
            $cpcBid->setBid($money);
            $biddingStrategyConfiguration->setBids(array($cpcBid));
     ...
     */
    public static function asNegativeAdGroupCriterion($adGroupId, ProductPartition $productPartition)
    {
     ...
     */
    private static function createAdGroupCriterionOperation(AdGroupCriterion $criterion, $operator)
    {
     ...
     */
    public static function showAdGroupTree(AdWordsServices $adWordsServices, AdWordsSession $session, $adGroupId)
    {
        ...
        $offset = 0;
        do {
            $query = sprintf('SELECT AdGroupId, Id, ParentCriterionId, CaseValue, PartitionType' . ' WHERE AdGroupId = %s AND CriteriaType = PRODUCT_PARTITION' . ' AND Status IN [ENABLED, PAUSED]', $adGroupId);
            ...
            if ($page->getEntries() !== null) {
                ...
                foreach ($page->getEntries() as $adGroupCriterion) {
                    ...
                    if (!array_key_exists($criterionId, $children)) {
                        $children[$criterionId] = array();
                    }
                    if ($adGroupCriterion->getCriterion()->getParentCriterionId() !== null) {
                        $parentCriterionId = $adGroupCriterion->getCriterion()->getParentCriterionId();
     ...
     */
    private static function traverseTree(Criterion $node, array $children, $level)
    {
        $type = 'n/a';
        $value = 'n/a';
        if ($node->getCaseValue() !== null) {
            $type = ($tmp = new ReflectionClass($node->getCaseValue())) ? $tmp->getShortName() : $tmp->getShortName();
            if ($node->getCaseValue() instanceof ProductCanonicalCondition) {
                $value = $node->getCaseValue()->getCondition();
            } elseif ($node->getCaseValue() instanceof ProductBiddingCategory) {
                $value = sprintf('%s (%s)', $node->getCaseValue()->getType(), $node->getCaseValue()->getValue());
            } else {
                $value = $node->getCaseValue()->getValue();
            }
        }
        $result = sprintf('%sID: %d, type: %s, value: %s
', str_repeat('  ', $level), $node->getId(), $type, $value);
