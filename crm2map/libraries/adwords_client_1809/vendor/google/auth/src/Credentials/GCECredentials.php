...
class GCECredentials extends CredentialsLoader
{
    ...
    public static function onGce(callable $httpHandler = null)
    {
        ...
        for ($i = 1; $i <= self::MAX_COMPUTE_PING_TRIES; $i++) {
            try {
                ...
                // the metadata resolution was particularly slow. The latter case is
                // "unlikely".
                $resp = $httpHandler(new Request('GET', $checkUri), array('timeout' => self::COMPUTE_PING_CONNECTION_TIMEOUT_S));
    ...
    public function fetchAuthToken(callable $httpHandler = null)
    {
        ...
        $resp = $httpHandler(new Request('GET', self::getTokenUri(), array(self::FLAVOR_HEADER => 'Google')));
    ...
    public function getLastReceivedToken()
    {
        if ($this->lastReceivedToken) {
            return array('access_token' => $this->lastReceivedToken['access_token'], 'expires_at' => $this->lastReceivedToken['expires_at']);
        }
        return null;
    }
}