...
class ScopedAccessTokenMiddleware
{
    ...
    public function __construct(callable $tokenFunc, $scopes, array $cacheConfig = null, CacheItemPoolInterface $cache = null)
    {
        ...
        if (!is_null($cache)) {
            $this->cache = $cache;
            $this->cacheConfig = array_merge(array('lifetime' => self::DEFAULT_CACHE_LIFETIME, 'prefix' => ''), $cacheConfig);
        }
    }
    /**
    ...
    public function __invoke(callable $handler)
    {
        ...
        return function (RequestInterface $request, array $options) use($handler, $that) {
            ...
            $request = $request->withHeader('authorization', 'Bearer ' . $that->fetchToken());
            return $handler($request, $options);
        };
    }
    /**
