...
class MockHandler implements \Countable
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        return $response->then(function ($value) use($request, $options, $that) {
            $that->invokeStats($request, $options, $value);
            if ($that->onFulfilled) {
                call_user_func($that->onFulfilled, $value);
            }
            ...
            return $value;
        }, function ($reason) use($request, $options, $that) {
            $that->invokeStats($request, $options, null, $reason);
            if ($that->onRejected) {
                call_user_func($that->onRejected, $reason);
            }
