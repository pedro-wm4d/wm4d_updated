...
class Client implements ClientInterface
{
    ...
    {
        $defaults = array('allow_redirects' => RedirectMiddleware::$defaultSettings, 'http_errors' => true, 'decode_content' => true, 'verify' => false, 'cookies' => false);
    ...
    {
        $modify = array('set_headers' => array());
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = Psr7\_caseless_remove(array('Content-Type'), $options['_conditional']);
        ...
        if (!empty($options['decode_content']) && $options['decode_content'] !== true) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = Psr7\_caseless_remove(array('Accept-Encoding'), $options['_conditional']);
        ...
        if (!empty($options['auth']) && is_array($options['auth'])) {
            ...
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = Psr7\_caseless_remove(array('Authorization'), $modify['set_headers']);
