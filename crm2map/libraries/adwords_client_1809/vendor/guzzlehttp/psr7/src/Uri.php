...
class Uri implements UriInterface
{
    ...
    private static $defaultPorts = array('http' => 80, 'https' => 443, 'ftp' => 21, 'gopher' => 70, 'nntp' => 119, 'news' => 119, 'telnet' => 23, 'tn3270' => 23, 'imap' => 143, 'pop' => 110, 'ldap' => 389);
    ...
    {
        return $uri->getScheme() === '' && $uri->getAuthority() === '' && (($tmp = $uri->getPath()) ? isset($tmp[0]) : isset($tmp[0])) && (($tmp = $uri->getPath()) ? $tmp[0] : $tmp[0]) === '/';
    }
    /**
    ...
    {
        $result = self::getFilteredQueryString($uri, array($key));
    ...
    private static function getFilteredQueryString(UriInterface $uri, array $keys)
    {
        ...
        return array_filter(explode('&', $current), function ($part) use($decodedKeys) {
            return !in_array(rawurldecode(($tmp = explode('=', $part)) ? $tmp[0] : $tmp[0]), $decodedKeys, true);
        });
    }
    /**
     * @param string      $key
    ...
    private function validateState()
    {
        ...
        if ($this->getAuthority() === '') {
            ...
            if ($this->scheme === '' && false !== strpos(($tmp = explode('/', $this->path, 2)) ? $tmp[0] : $tmp[0], ':')) {
