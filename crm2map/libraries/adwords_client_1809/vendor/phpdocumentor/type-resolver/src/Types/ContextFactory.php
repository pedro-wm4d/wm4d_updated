...
final class ContextFactory
{
    ...
    public function createFromReflector(\Reflector $reflector)
    {
        ...
        return new Context($namespace, array());
    }
    /**
    ...
    public function createForNamespace($namespace, $fileContents)
    {
        ...
        while ($tokens->valid()) {
            switch (($tmp = $tokens->current()) ? $tmp[0] : $tmp[0]) {
                case T_NAMESPACE:
                ...
                case T_CLASS:
                    ...
                    while ($tokens->valid() && ($braceLevel > 0 || !$firstBraceFound)) {
                        if ($tokens->current() === '{' || (($tmp = $tokens->current()) ? $tmp[0] : $tmp[0]) === T_CURLY_OPEN || (($tmp = $tokens->current()) ? $tmp[0] : $tmp[0]) === T_DOLLAR_OPEN_CURLY_BRACES) {
                            if (!$firstBraceFound) {
    ...
    private function parseNamespace(\ArrayIterator $tokens)
    {
        ...
        $name = '';
        while ($tokens->valid() && ((($tmp = $tokens->current()) ? $tmp[0] : $tmp[0]) === T_STRING || (($tmp = $tokens->current()) ? $tmp[0] : $tmp[0]) === T_NS_SEPARATOR)) {
            $name .= ($tmp = $tokens->current()) ? $tmp[1] : $tmp[1];
            $tokens->next();
        }
        return $name;
    }
    ...
    private function parseUseStatement(\ArrayIterator $tokens)
    {
        ...
        while ($continue) {
            ...
            $uses[$alias] = $fqnn;
            if ((($tmp = $tokens->current()) ? $tmp[0] : $tmp[0]) === self::T_LITERAL_END_OF_USE) {
                $continue = false;
            }
        }
        return $uses;
    ...
    {
        $result = array('');
        while ($tokens->valid() && (($tmp = $tokens->current()) ? $tmp[0] : $tmp[0]) !== self::T_LITERAL_USE_SEPARATOR && (($tmp = $tokens->current()) ? $tmp[0] : $tmp[0]) !== self::T_LITERAL_END_OF_USE) {
            if ((($tmp = $tokens->current()) ? $tmp[0] : $tmp[0]) === T_AS) {
                $result[] = '';
            }
            if ((($tmp = $tokens->current()) ? $tmp[0] : $tmp[0]) === T_STRING || (($tmp = $tokens->current()) ? $tmp[0] : $tmp[0]) === T_NS_SEPARATOR) {
                $result[count($result) - 1] .= ($tmp = $tokens->current()) ? $tmp[1] : $tmp[1];
            }
            $tokens->next();
        }
        if (count($result) == 1) {
