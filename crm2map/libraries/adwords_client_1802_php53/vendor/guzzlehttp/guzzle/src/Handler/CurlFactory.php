...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function finishError(callable $handler, EasyHandle $easy, CurlFactoryInterface $factory)
    {
        ...
        $ctx = array('errno' => $easy->errno, 'error' => curl_error($easy->handle)) + curl_getinfo($easy->handle);
        $factory->release($easy);
    ...
    {
        static $connectionErrors = array(CURLE_OPERATION_TIMEOUTED => true, CURLE_COULDNT_RESOLVE_HOST => true, CURLE_COULDNT_CONNECT => true, CURLE_SSL_CONNECT_ERROR => true, CURLE_GOT_NOTHING => true);
    ...
    {
        $conf = array('_headers' => $easy->request->getHeaders(), CURLOPT_CUSTOMREQUEST => $easy->request->getMethod(), CURLOPT_URL => (string) $easy->request->getUri()->withFragment(''), CURLOPT_RETURNTRANSFER => false, CURLOPT_HEADER => false, CURLOPT_CONNECTTIMEOUT => 150);
