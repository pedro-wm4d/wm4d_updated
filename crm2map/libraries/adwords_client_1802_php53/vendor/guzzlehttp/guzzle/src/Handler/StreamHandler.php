...
class StreamHandler
{
    ...
    private function invokeStats(array $options, RequestInterface $request, $startTime, ResponseInterface $response = null, $error = null)
    {
        if (isset($options['on_stats'])) {
            $stats = new TransferStats($request, $response, microtime(true) - $startTime, $error, array());
    ...
    private function createResponse(RequestInterface $request, array $options, $stream, $startTime)
    {
        ...
        $ver = ($tmp = explode('/', $parts[0])) ? $tmp[1] : $tmp[1];
        $status = $parts[1];
    ...
    private function createStream(RequestInterface $request, array $options)
    {
        ...
        return $this->createResource(function () use($uri, &$http_response_header, $context, $options, $that) {
    ...
    private function getDefaultContext(RequestInterface $request)
    {
        ...
        $context = array('http' => array('method' => $request->getMethod(), 'header' => $headers, 'protocol_version' => $request->getProtocolVersion(), 'ignore_errors' => true, 'follow_location' => 0));
...
');
    ...
    private function addNotification(array &$params, callable $notify)
    {
        ...
        if (!isset($params['notification'])) {
            ...
            $params['notification'] = $this->callArray(array($params['notification'], $notify));
        }
    }
