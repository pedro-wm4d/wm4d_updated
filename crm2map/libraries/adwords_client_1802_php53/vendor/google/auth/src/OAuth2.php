...
class OAuth2 implements FetchAuthTokenInterface
{
    ...
    {
        $opts = array_merge(array('expiry' => self::DEFAULT_EXPIRY_SECONDS, 'extensionParams' => array(), 'authorizationUri' => null, 'redirectUri' => null, 'tokenCredentialUri' => null, 'state' => null, 'username' => null, 'password' => null, 'clientId' => null, 'clientSecret' => null, 'issuer' => null, 'sub' => null, 'audience' => null, 'signingKey' => null, 'signingAlgorithm' => null, 'scope' => null, 'additionalClaims' => array()), $config);
     ...
     * @return string
     */
    public function toJwt(array $config = array())
    {
        ...
        $now = time();
        $opts = array_merge(array('skew' => self::DEFAULT_SKEW_SECONDS), $config);
        $assertion = array('iss' => $this->getIssuer(), 'aud' => $this->getAudience(), 'exp' => $now + $this->getExpiry(), 'iat' => $now - $opts['skew']);
    ...
    public function generateCredentialsRequest()
    {
        ...
        /*
        echo "$uri\n";
        print_r(Psr7\build_query($params));exit;
        */
    ...
    {
        $opts = array_merge(array('extensionParams' => array(), 'access_token' => null, 'id_token' => null, 'expires_in' => null, 'expires_at' => null, 'issued_at' => null), $config);
     ...
     */
    public function buildFullAuthorizationUri(array $config = array())
    {
        if (is_null($this->getAuthorizationUri())) {
            throw new InvalidArgumentException('requires an authorizationUri to have been set');
        }
        $params = array_merge(array('response_type' => 'code', 'access_type' => 'offline', 'client_id' => $this->clientId, 'redirect_uri' => $this->redirectUri, 'state' => $this->state, 'scope' => $this->getScope()), $config);
    ...
    public function getLastReceivedToken()
    {
        if ($token = $this->getAccessToken()) {
            return array('access_token' => $token, 'expires_at' => $this->getExpiresAt());
        }
        return null;
    }
    /**
