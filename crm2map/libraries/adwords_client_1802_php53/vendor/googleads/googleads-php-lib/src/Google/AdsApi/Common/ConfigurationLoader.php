...
final class ConfigurationLoader
{
     ...
     */
    public function __construct(EnvironmentalVariables $environmentalVariables = null)
    {
        $this->environmentalVariables = $environmentalVariables === null ? new EnvironmentalVariables() : $environmentalVariables;
    }
    /**
    ...
    public function fromFile($configIniFilePath)
    {
        if (!file_exists($configIniFilePath)) {
            try {
                ...
                $homeConfigIniFilePath = $home . DIRECTORY_SEPARATOR . pathinfo($configIniFilePath, PATHINFO_BASENAME);
                if (file_exists($homeConfigIniFilePath)) {
                    $configIniFilePath = $homeConfigIniFilePath;
                } else {
                    throw new InvalidArgumentException(sprintf('Config file not found as specified: \'%s\' or in the home ' . 'directory: \'%s\'.', $configIniFilePath, $homeConfigIniFilePath));
                }
            } catch (UnexpectedValueException $e) {
                throw new InvalidArgumentException(sprintf('Config file not found as specified: \'%s\'. Home ' . 'directory could not be located so it was not searched.', $configIniFilePath));
            }
        }
