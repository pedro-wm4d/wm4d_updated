...
final class AdsGuzzleHttpClientFactory implements GuzzleHttpClientFactory
{
     ...
     */
    public function __construct(LoggerInterface $logger, GuzzleLogMessageFormatter $messageFormatter, Client $httpClient = null)
    {
        $this->logger = $logger;
        $this->messageFormatter = $messageFormatter;
        $this->config = $httpClient === null ? array() : $httpClient->getConfig();
    }
    /**
    ...
    {
        $config = $this->config;
        if (!array_key_exists('handler', $config) || $config['handler'] === null) {
            $config['handler'] = HandlerStack::create();
        }
        // Add a logging middleware required by this library.
        $config['handler']->before('http_errors', GuzzleLogMessageHandler::log($this->logger, $this->messageFormatter));
