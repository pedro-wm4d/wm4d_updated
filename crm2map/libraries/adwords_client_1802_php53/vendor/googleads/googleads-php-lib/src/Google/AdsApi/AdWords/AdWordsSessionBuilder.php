...
final class AdWordsSessionBuilder implements AdsBuilder
{
    ...
    private static $DEFAULT_REPORT_DOWNLOADER_LOGGER_CHANNEL = 'AW_REPORT_DOWNLOADER';
    ...
    {
        $this->developerToken = $configuration->getConfiguration('developerToken', 'ADWORDS');
        ...
        $this->clientCustomerId = $configuration->getConfiguration('clientCustomerId', 'ADWORDS');
        if ($configuration->getConfiguration('isPartialFailure', 'ADWORDS') !== null) {
            $this->isPartialFailure = boolval($configuration->getConfiguration('isPartialFailure', 'ADWORDS'));
        }
        if ($configuration->getConfiguration('includeUtilitiesInUserAgent', 'ADWORDS') !== null) {
            $this->isIncludeUtilitiesInUserAgent = boolval($configuration->getConfiguration('includeUtilitiesInUserAgent', 'ADWORDS'));
        }
        $this->reportSettings = $this->reportSettingsBuilder->from($configuration)->build();
        //echo "in";exit;
        $this->soapLogger = $this->adsLoggerFactory->createLogger(self::$DEFAULT_SOAP_LOGGER_CHANNEL, $configuration->getConfiguration('soapLogFilePath', 'LOGGING'), $configuration->getConfiguration('soapLogLevel', 'LOGGING'));
        $this->reportDownloaderLogger = $this->adsLoggerFactory->createLogger(self::$DEFAULT_REPORT_DOWNLOADER_LOGGER_CHANNEL, $configuration->getConfiguration('reportDownloaderLogFilePath', 'LOGGING'), $configuration->getConfiguration('reportDownloaderLogLevel', 'LOGGING'));
        $this->batchJobsUtilLogger = $this->adsLoggerFactory->createLogger(self::$DEFAULT_BJ_UTIL_LOGGER_CHANNEL, $configuration->getConfiguration('batchJobsUtilLogFilePath', 'LOGGING'), $configuration->getConfiguration('batchJobsUtilLogLevel', 'LOGGING'));
        return $this;
    }
    /**
     ...
     * @return AdWordsSessionBuilder this builder
     */
    public function withConnectionSettings(ConnectionSettings $connectionSettings)
    {
     ...
     * @return AdWordsSessionBuilder this builder
     */
    public function withAdsHeaderFormatter(AdsHeaderFormatter $adsHeaderFormatter)
    {
     ...
     * @return AdWordsSessionBuilder this builder
     */
    public function withReportDownloaderLogger(LoggerInterface $reportDownloaderLogger)
    {
     ...
     * @return AdWordsSessionBuilder this builder
     */
    public function withBatchJobsUtilLogger(LoggerInterface $batchJobsUtilLogger)
    {
    ...
    public function defaultOptionals()
    {
        ...
        if ($this->reportSettings === null) {
            $this->reportSettings = ($tmp = new ReportSettingsBuilder()) ? $tmp->build() : $tmp->build();
        }
        if ($this->connectionSettings === null) {
            $this->connectionSettings = ($tmp = new ConnectionSettingsBuilder()) ? $tmp->build() : $tmp->build();
        }
        if ($this->soapSettings === null) {
            $this->soapSettings = ($tmp = new SoapSettingsBuilder()) ? $tmp->build() : $tmp->build();
        }
        if ($this->soapLogger === null) {
            $this->soapLogger = $this->adsLoggerFactory->createLogger(self::$DEFAULT_SOAP_LOGGER_CHANNEL);
        }
        if ($this->reportDownloaderLogger === null) {
            $this->reportDownloaderLogger = $this->adsLoggerFactory->createLogger(self::$DEFAULT_REPORT_DOWNLOADER_LOGGER_CHANNEL);
        }
        if ($this->batchJobsUtilLogger === null) {
            $this->batchJobsUtilLogger = $this->adsLoggerFactory->createLogger(self::$DEFAULT_BJ_UTIL_LOGGER_CHANNEL);
        }
    }
    /**
    ...
    public function validate()
    {
        ...
        if ($this->userAgent === null || trim($this->userAgent) === '') {
            ...
            throw new InvalidArgumentException('User agent must contain only ASCII characters.');
        }
        ...
        if ($this->oAuth2Credential === null) {
            throw new InvalidArgumentException('Missing OAuth2 authentication credentials.');
        }
    }
    /**
