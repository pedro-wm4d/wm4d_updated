...
final class SoapHeaders
{
    ...
    public static function getSoapHeaderValue($xml, $soapHeaderName)
    {
        ...
        while ($xmlReader->read()) {
            if ($xmlReader->nodeType === XMLReader::ELEMENT && $xmlReader->localName === $soapHeaderName) {
                $xmlReader->read();
                $headerValue = $xmlReader->value;
            } else {
                if ($xmlReader->nodeType === XMLReader::END_ELEMENT && $xmlReader->localName === self::$SOAP_HEADER_NODE_NAME) {
                    break;
                }
            }
        }
    ...
    public static function getSoapResponseHeaderValues($xml)
    {
        ...
        $xmlReader->xml($xml);
        while ($xmlReader->read()) {
            if ($xmlReader->nodeType === XMLReader::ELEMENT && $xmlReader->localName === self::$SOAP_RESPONSE_HEADER_NODE_NAME) {
                $foundOpenTag = true;
                continue;
            } else {
                if ($xmlReader->nodeType === XMLReader::END_ELEMENT && $xmlReader->localName === self::$SOAP_RESPONSE_HEADER_NODE_NAME) {
                    break;
                } else {
                    if ($foundOpenTag && $xmlReader->nodeType === XMLReader::ELEMENT) {
