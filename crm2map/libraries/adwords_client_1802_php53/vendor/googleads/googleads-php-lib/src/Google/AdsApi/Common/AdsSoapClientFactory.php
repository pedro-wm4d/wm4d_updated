...
{
    const DEFAULT_SOAP_CALL_TIMEOUT = 600;
    // 60 seconds * 10 -> 10 minutes
    private $reflection;
     ...
     */
    public function __construct(SoapLogMessageFormatter $soapLogMessageFormatter, Reflection $reflection = null, $soapCallTimeout = null)
    {
        $this->soapLogMessageFormatter = $soapLogMessageFormatter;
        $this->reflection = $reflection === null ? new Reflection() : $reflection;
        $this->soapCallTimeout = $soapCallTimeout === null ? self::DEFAULT_SOAP_CALL_TIMEOUT : $soapCallTimeout;
    }
    /**
     ...
     */
    public function generateSoapClient(AdsSession $session, AdsHeaderHandler $headerHandler, AdsServiceDescriptor $serviceDescriptor)
    {
        $options = array('trace' => true, 'encoding' => 'utf-8', 'connection_timeout' => $this->soapCallTimeout, 'features' => SOAP_SINGLE_ELEMENT_ARRAYS);
        $soapSettings = $session->getSoapSettings();
        $options = $this->populateOptions($session, $options, $soapSettings);
        $soapClient = $this->reflection->createInstance($serviceDescriptor->getServiceClass(), $options);
        ...
        $soapClient->__setLocation(str_replace(sprintf('%s://%s', parse_url($soapClient->getWsdlUri(), PHP_URL_SCHEME), parse_url($soapClient->getWsdlUri(), PHP_URL_HOST)), rtrim($session->getEndpoint(), '/'), $soapClient->getWsdlUri()));
        return $soapClient;
    }
    private function populateOptions(AdsSession $session, array $options, SoapSettings $soapSettings = null)
    {
        $contextOptions = array();
        if ($soapSettings !== null) {
            // Compression settings.
            if ($soapSettings->getCompressionLevel() !== null) {
                $options['compression'] = SOAP_COMPRESSION_ACCEPT | SOAP_COMPRESSION_GZIP | $soapSettings->getCompressionLevel();
            ...
            if ($soapSettings->getWsdlCacheType() !== null) {
                trigger_error('WSDL caching is already deprecated in favor of WSDLs ' . 'bundled with this library.
', E_USER_WARNING);
            }
            // Proxy settings.
            list($proxyLogin, $proxyPassword, $proxyHost, $proxyPort) = $session->getConnectionSettings()->getProxyParts();
            if (!empty($proxyHost)) {
                ...
                if (PHP_VERSION_ID < 50600) {
                    if (!isset($contextOptions['ssl']['cafile'])) {
                        try {
                            ...
                            throw new RuntimeException('No system CA bundle could be found ' . 'in any of the the common system locations. PHP versions ' . 'earlier than 5.6 are not properly configured to use the ' . 'system\'s CA bundle by default. In order to verify peer ' . 'certificates, you will need to supply the path on disk to ' . 'a certificate bundle in the SoapSettings of an ' . 'AdsSession. See SoapSettingsBuilder::withSslCaFile().');
                        }
                    }
                    $contextOptions['ssl']['verify_peer'] = true;
                }
            } else {
                $session->getSoapLogger()->warning('Disabling SSL verification is not recommended.');
