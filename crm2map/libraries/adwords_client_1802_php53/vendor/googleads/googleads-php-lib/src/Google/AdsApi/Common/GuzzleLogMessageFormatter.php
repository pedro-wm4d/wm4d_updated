...
';
     ...
     */
    public function __construct(array $requestHttpHeadersToScrub, array $additionalSummaryInfo, $shouldLogResponsePayload, $redactedResponsePayloadMessage = null, MessageFormatter $summaryMessageFormatter = null, MessageFormatter $detailedMessageFormatter = null)
    {
        self::$SUMMARY_MESSAGE_FORMAT = '{req_header_User-Agent} "{method} {target} HTTP/{version}" Status: {code} ';
        ...
        $this->redactedResponsePayloadMessage = $redactedResponsePayloadMessage === null ? self::$REDACTED_DATA_MESSAGE : $redactedResponsePayloadMessage;
        $this->summaryMessageFormatter = $summaryMessageFormatter === null ? new MessageFormatter(self::$SUMMARY_MESSAGE_FORMAT) : $summaryMessageFormatter;
        if ($this->shouldLogResponsePayload === true) {
            $guzzleMessageFormat = sprintf(self::$DETAILED_MESSAGE_FORMAT, '{response}');
        } else {
            $guzzleMessageFormat = sprintf(self::$DETAILED_MESSAGE_FORMAT, $this->redactedResponsePayloadMessage);
        }
        $this->detailedMessageFormatter = $detailedMessageFormatter === null ? new MessageFormatter($guzzleMessageFormat) : $detailedMessageFormatter;
    }
    /**
     ...
     */
    public function formatSummary(RequestInterface $request, ResponseInterface $response = null)
    {
        return sprintf('%s %s', http_build_query($this->additionalSummaryInfo, '', ' '), $this->summaryMessageFormatter->format($request, $response));
    }
    /**
     ...
     */
    public function formatDetailed(RequestInterface $request, ResponseInterface $response = null, Exception $error = null)
    {
        $requestHeaders = LogMessageScrubbers::scrubHttpHeadersArray($request->getHeaders(), $this->requestHttpHeadersToScrub);
        $changes = array('set_headers' => $requestHeaders, 'body' => urldecode($request->getBody()));
        $clonedRequest = \GuzzleHttp\Psr7\modify_request($request, $changes);
        return $this->detailedMessageFormatter->format($clonedRequest, $response, $error);
    }
}