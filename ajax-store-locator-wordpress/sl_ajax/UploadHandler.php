...
class UploadHandler
{
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        $index = null, $content_range = null, $StoreId) {
        $file = new stdClass();
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
					...
					$full_Path = "gallery/".$StoreId."/".$file->name;
					$sql_qry = "INSERT INTO `storegallery`(`StoreId`, `ImgUrl`, `IsNew`, `CreatedDate`) VALUES ($StoreId, '$full_Path', '1', '$modifiedDate')";
	...
	//print_r($_REQUEST);
        ...
        if ($upload && is_array($upload['tmp_name'])) {
			...
			//echo 'yes';
			// $textboxarr = array();
			// for($i = 0;$i<count($_POST['tbTitle']);$i++){
					 // $textboxarr = $_POST['tbTitle'][$i];
			// }
			$count_i =0;
			//print_r($textboxarr);
            foreach ($upload['tmp_name'] as $index => $value) {
				//$title = $_REQUEST["tbTitle"][$count_i];
				//echo $value;
