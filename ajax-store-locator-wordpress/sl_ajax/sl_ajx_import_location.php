...
function sl_dal_locationimport_cbf(){
	...
	if(isset($_POST["Funtype"])){
		...
		if($funType == "Import_Location"){
			...
			$SavedCount = 0;
			$totRecord = 0;
			$headArray = array("iscustid", "Name", "Address", "City", "State", "Country", "Zip_code", "Phone", "Fax", "Email", "Website", "Category", "Status");
			...
			foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
				if($sheet_count == 0){
					...
					for ($row = 8; $row <= $highestRow; ++ $row) {
						...
						$storeiscustid 		= ( $worksheet->getCellByColumnAndRow(0, $row)->getValue() != "" )  ? mysql_real_escape_string( trim($worksheet->getCellByColumnAndRow(0, $row)->getValue()) ) : "";
						...
						if($my_numresult > 0 && $coords['stat'] == 'OK'){
							...
							(`iscustid`,`name`, `address`, `lat`, `lng`, `city`, `state`, `country`, `zip_code`, `phone`, `fax`, `email`, `website`, `type`, `logoid`, `logotype`, `labelid`, `labeltext`)
							VALUES ('".$storeiscustid."', '".$storeName."', '".$storeAdd."', '".$coords['lat']."', '".$coords['lng']."', '".$storeCity."', '".$storeState."', '".$storeCoun."', '$storeZip', '$storePhone', '$storeFax', '$storeEmail', '$storeWeb', '$cateId', '$DefaultLogoId', 'D', '1', '')";
